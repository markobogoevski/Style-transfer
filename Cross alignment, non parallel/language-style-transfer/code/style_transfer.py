...
from .utils import *
from .nn import *
from . import beam_search, greedy_decoding
...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        self.loss_g = tf.reduce_sum(loss_g) / tf.compat.v1.to_float(self.batch_size)
        ...
        self.optimizer_ae = tf.compat.v1.train.AdamOptimizer(self.learning_rate,
                                                             beta1, beta2).minimize(self.loss_g, var_list=theta_eg)
        self.optimizer_d0 = tf.compat.v1.train.AdamOptimizer(self.learning_rate,
                                                             beta1, beta2).minimize(self.loss_d0, var_list=theta_d0)
        self.optimizer_d1 = tf.compat.v1.train.AdamOptimizer(self.learning_rate,
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        print(f'#sents of training file 0: {len(train0)}')
        print(f'#sents of training file 1: {len(train1)}')
    ...
    with tf.compat.v1.Session(config=config) as sess:
            ...
            for epoch in range(1, 1 + args.max_epochs):
                ...
                for batch in batches:
                    ...
                    if step % args.steps_per_checkpoint == 0:
                        ori, tsf = decoder.rewrite(batch)
                        print(f'original: {" ".join(w for w in ori[0])}')
                        print(f'transfer: {" ".join(w for w in tsf[0])}')
        ...
        if args.online_testing:
            while True:
                ...
                ori, tsf = decoder.rewrite(batch)
                index = random.randrange(0, len(ori))
                print(f'original: {" ".join(w for w in ori[index])}')
                print(f'transfer: {" ".join(w for w in tsf[index])}')
